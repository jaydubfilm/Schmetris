...
//Bullet base class
    public class Bullet : MonoBehaviour
    {
        ...
        [ShowIf("isGrenade", true)] public float blastRadius;
        [ShowIf("isGrenade", true)] public float blastForce;
        ...
        void Update()
        {
            ...
            if (transform.position.y > ScreenStuff.topEdgeOfWorld ||
                transform.position.y < ScreenStuff.bottomEdgeOfWorld ||
                transform.position.x > ScreenStuff.rightEdgeOfWorld ||
                transform.position.x < ScreenStuff.leftEdgeOfWorld)
                Destroy(gameObject);
                ...
                if (isGrenade)
                {
                    ...
                    for (int i = 0; i < colliders.Length; i++)
                    {
                        if (colliders[i].GetComponent<Enemy>())
                        {
                            ...
                            GameObject debugSphereInstance =
                                Instantiate(debugSphere, transform.position, Quaternion.identity);
                            debugSphereInstance.transform.localScale =
                                new Vector3(blastRadius * 2, blastRadius * 2, blastRadius * 2);
                        }
                    }
                }
