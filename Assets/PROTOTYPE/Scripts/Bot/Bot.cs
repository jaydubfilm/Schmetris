...
namespace StarSalvager.Prototype
{
    ...
    public class Bot : MonoBehaviour
    {
        ...
        [HideInInspector] public Quaternion rotation1;

        [HideInInspector] public Quaternion rotation2;

        bool isRotating = false;

        [HideInInspector] public Vector2Int[] directionV2Arr = new[]
        {
            new Vector2Int(0, 1),
            new Vector2Int(1, 0),
            new Vector2Int(0, -1),
        ...
        public float storedGrey
        {
            ...
            get { return _storedGrey; }
        }

        float totalResources
        {
            get { return storedRed + storedBlue + storedGreen + storedYellow + storedGrey; }
        }
        ...
        void OnGameRestart()
        {
            ...
            startTileMap = Instantiate(startingBrickGrid.GetComponent<Tilemap>(), new Vector3(0, 0, 0),
                Quaternion.identity);
            AddStartingBricks();
        ...
        public void OnLevelRestart()
        {
            ...
            {
                BrickAtBotArr(containerData.coords).GetComponent<Container>()
                    .SetOpenDirection(containerData.openDirection, true);
            }
        ...
        void Update()
        {
            ...
            Brick coreBurn = (!GameController.Instance.isLevelCompleteQueued && BrickAtBotArr(coreV2))
                ? BrickAtBotArr(coreV2).GetComponent<Brick>()
                : null;
            if (coreBurn && !coreBurn.hasResources)
            {
                ...
                {
                    fuelBrickList[0].GetComponent<Fuel>()
                        .BurnFuel(coreBurn.redBurn[coreBurn.GetPoweredLevel()] * Time.deltaTime);
                }
        ...
        public void BumpColumn(Vector2Int startArrPos, Vector2Int bumpDirV2)
        {
            ...
            if ((IsValidBrickPos(startArrPos) == false) || (BrickAtBotArr(startArrPos) == null) ||
                (startArrPos == coreV2))
                return;
            ...
            Brick resource = null;
            while (bumpDirV2 != Vector2.zero && resource == null && (IsValidScreenPos(endCoords + bumpDirV2)) &&
            ...
            if (resource)
            {
                AddResource(BrickAtScreenArr(endCoords + bumpDirV2).GetComponent<Brick>(), resource.brickType,
                    resource.brickLevel);
            ...
            for (int l = length; l > 0; l--)
            {
                ...
                BrickAtScreenArr(brickArrPos).GetComponent<Brick>()
                    .MoveBrick(ScreenToBotCoords(brickArrPos + bumpDirV2));
            }
        ...
        public bool IsSquareComplete(int squareNumber)
        {
            ...
            for (int x = -squareNumber; x <= squareNumber; x++)
                if ((brickTypeArr[maxBotRadius + x, maxBotRadius + squareNumber] == -1) ||
                    (brickTypeArr[maxBotRadius + x, maxBotRadius - squareNumber] == -1))
                    squareIsComplete = false;
            // check sides
            for (int y = 1 - squareNumber; y <= squareNumber - 1; y++)
                if ((brickTypeArr[maxBotRadius - squareNumber, maxBotRadius + y] == -1) ||
                    (brickTypeArr[maxBotRadius + squareNumber, maxBotRadius + y] == -1))
                    squareIsComplete = false;
        ...
        public void TripleTestBot()
        {
            ...
            {
                if (!brickObj.GetComponent<Brick>().IsParasite() && !brickObj.GetComponent<CraftedPart>() &&
                    TripleTestBrick(brickObj.GetComponent<Brick>().arrPos) == true)
                {
                    return;
                }
            }
        ...
        public bool TripleTestBrick(Vector2Int arrPos)
        {
            ...
            if ((sideBrick1 && !sideBrick1.GetComponent<Brick>().IsParasite() && IsConnectedToCore(sideBrick1)) ||
                (sideBrick2 && !sideBrick2.GetComponent<Brick>().IsParasite() && IsConnectedToCore(sideBrick2)))
                centreIsStable = true;
        ...
        void RepositionUpgradeOrphans(bool isHorizontal, Vector2Int upgradeFinal, List<GameObject> setA,
            List<GameObject> setB)
        {
            if (isHorizontal)
            {
                foreach (GameObject OrphanA in setA)
                {
                    ...
                    if (orphanBrick)
                    {
                        orphanBrick.MoveBrick(new Vector2Int(
                            orphanBrick.arrPos.x + (upgradeFinal.x > orphanBrick.arrPos.x ? 1 : -1),
                            orphanBrick.arrPos.y));
                    }
                }
            ...
            else
            {
                foreach (GameObject OrphanA in setA)
                {
                    ...
                    if (orphanBrick)
                    {
                        orphanBrick.MoveBrick(new Vector2Int(orphanBrick.arrPos.x,
                            orphanBrick.arrPos.y + (upgradeFinal.y > orphanBrick.arrPos.y ? 1 : -1)));
                    }
                }
            ...
            if (IsValidBrickPos(arrPos1) && IsValidBrickPos(arrPos2) && IsValidBrickPos(arrPos3))
            {
                ...
                if ((brick1 != null) && (brick2 != null) && (brick3 != null))
                {
                    ...
                    if ((testType1 == testType2) && (testType2 == testType3)
                                                 && (testBrickLevel1 == testBrickLevel2) &&
                                                 (testBrickLevel2 == testBrickLevel3))
                        return true;
                    else
                        return false;
                }
        ...
        public Vector2Int GetNearestValidBrickPos(Vector2Int arrPos, bool isRecursing)
        {
            ...
            for (int x = 0; x < 4; x++)
            {
                ...
                if (IsValidBrickPos(testCoords) && BrickAtBotArr(testCoords) != null &&
                    !BrickAtBotArr(testCoords).GetComponent<Brick>().IsParasite() && testCoords != coreV2)
                {
                    hasNeighbor = true;
                }
            }
        ...
        {
            if ((0 <= mergedArrPos.x) && (mergedArrPos.x < maxBotWidth + 4) && (0 <= mergedArrPos.y) &&
                (mergedArrPos.y < maxBotHeight + 4))
                return true;
            else
                return false;
        }
        ...
        public void ConvertBricksToBlock(List<GameObject> group)
        {
            ...
            foreach (GameObject orphanBrickObj in group)
            {
                ...
                if (orphanBrick.IsParasite())
                {
                    ...
                    newEnemyObj = Instantiate(GameController.Instance.enemyReference[type],
                        orphanBrick.transform.position, Quaternion.identity);
        ...
        public Vector3 BotCoordsToScreenPos(Vector2Int botCoords)
        {
            ...
            Vector3 screenPos = transform.position +
                                new Vector3(screenOffset.x * ScreenStuff.colSize, screenOffset.y * ScreenStuff.colSize);
            return screenPos;
        }
        ...
        public GameObject AddBrick(Vector2Int arrPos, int type, int level = 0)
        {
            ...
            newBrick = Object.Instantiate(masterBrickList[type], new Vector3(coreX, coreY, 0), Quaternion.identity,
        ...
        void TouchInputCheck()
        {
            ...
            else if (Input.GetMouseButton(0))
            {
                ...
                else
                {
                    ...
                    bool isBelowBot = Input.mousePosition.y <
                                      Camera.main.WorldToScreenPoint(coreBrick.transform.position).y;
                    bool isHorizontal = Mathf.Abs(bufferedMovePos.x - Input.mousePosition.x) >=
                                        Mathf.Abs(bufferedMovePos.y - Input.mousePosition.y);
        ...
        public float GetBurnRate(ResourceType resourceType)
        {
            ...
            foreach (Brick brickRef in resourceBurnBricks)
            {
                ...
                {
                    burnRate += brickRef.GetComponent<Repair>()
                        .GetConvertedBurnRate(resourceType, brickRef.GetPoweredLevel());
                }
                else if (brickRef.GetComponent<Gun>())
                {
                    burnRate += brickRef.GetComponent<Gun>()
                        .GetConvertedBurnRate(resourceType, brickRef.GetPoweredLevel());
                }
            }
        ...
        public bool CollisionCheck(int directionFlag, int vertFlag)
        {
            ...
            foreach (GameObject brickObj in brickList)
            {
                ...
                RaycastHit2D rH = Physics2D.BoxCast(brickObj.transform.position, Vector2.one * ScreenStuff.colSize, 0,
                    new Vector2(directionFlag, vertFlag), ScreenStuff.colSize, bitMask);
                if (rH.collider != null)
                {
                    Brick brick = brickObj.GetComponent<Brick>();
