    ...
    public class Enemy : MonoBehaviour
    {
        ...
        void ScoreEnemy()
        {
            ...
            if (data.redYield > 0)
            {
                ...
                GameController.Instance.CreateFloatingText(data.redYield.ToString(),
                    transform.position + new Vector3(1, 1, 0), 30, Color.red);
            }

            if (data.blueYield > 0)
            {
                GameController.Instance.bot.storedBlue += data.blueYield;
                GameController.Instance.CreateFloatingText(data.blueYield.ToString(),
                    transform.position + new Vector3(-1, 1, 0), 30, Color.blue);
            }

            if (data.yellowYield > 0)
            {
                GameController.Instance.bot.storedYellow += data.yellowYield;
                GameController.Instance.CreateFloatingText(data.yellowYield.ToString(),
                    transform.position + new Vector3(1, -1, 0), 30, Color.yellow);
            }

            if (data.greenYield > 0)
            {
                GameController.Instance.bot.storedGreen += data.greenYield;
                GameController.Instance.CreateFloatingText(data.greenYield.ToString(),
                    transform.position + new Vector3(-1, -1, 0), 30, Color.green);
            }

            if (data.greyYield > 0)
            {
                GameController.Instance.bot.storedGrey += data.greyYield;
                GameController.Instance.CreateFloatingText(data.greyYield.ToString(), transform.position, 30,
                    Color.grey);
            }
        }
        ...
        {
            rb2d.velocity = new Vector3(0, -GameController.Instance.blockSpeed * GameController.Instance.adjustedSpeed,
                0);
        }
