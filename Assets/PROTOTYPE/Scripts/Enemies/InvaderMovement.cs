...
//Space Invader type enemy
    public class InvaderMovement : Enemy
    {
        ...
        [Tooltip(
            "The distance frome the edge of the screen at which the Invader will reverse direction (screen width ranges 0 - 1)")]
        ...
        protected override void UpdateLiveBehaviour()
        {
            ...
            if (transform.position.x > player.position.x - fireOnProximity &&
                transform.position.x < player.position.x + fireOnProximity)
            {
        ...
        void ShiftDownOneRow()
        {
            transform.position = new Vector3(transform.position.x,
                transform.position.y - (ScreenStuff.rowSize * dropDownDistance));
        }
        ...
        void Fire()
        {
            EnemyBulletV2 thisBullet = Instantiate(bullet, transform.position, transform.rotation)
