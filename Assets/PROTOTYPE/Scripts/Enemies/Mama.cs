...
//Large Parasite-spawning enemy
    public class Mama : Enemy
    {
        ...
        protected override void UpdateDeathBehaviour()
        {
            ...
            {
                Color enemyColor = new Color(spriteRenderer.color.r,
                    1 - ((Time.timeSinceLevelLoad - timeOfDeath) / timeUntilFlashing),
                    1 - ((Time.timeSinceLevelLoad - timeOfDeath) / timeUntilFlashing));
        ...
        void Fire()
        {
            EnemyBulletV2 thisBullet = Instantiate(bullet, transform.position, Quaternion.identity)
                .GetComponent<EnemyBulletV2>();
            thisBullet.transform.localScale = new Vector3(thisBullet.transform.localScale.x * bulletScale,
                thisBullet.transform.localScale.y * bulletScale, thisBullet.transform.localScale.z * bulletScale);
        ...
        void DeathBlast()
        {
            Collider2D[] colliders =
                Physics2D.OverlapCircleAll(new Vector2(transform.position.x, transform.position.y), blastRadius);
            ...
            for (int i = 0; i < 4; i++)
            {
                Vector3 blastOffset = new Vector3(Random.Range(-blastRadius / 2.0f, blastRadius / 2.0f),
                    Random.Range(-blastRadius / 2.0f, blastRadius / 2.0f), 0);
                GameObject newParasite =
                    Instantiate(parasitePrefab, transform.position + blastOffset, Quaternion.identity);
            }
