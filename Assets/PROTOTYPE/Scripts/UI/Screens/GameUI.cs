...
//Manages HUD and other in-game UI
    public class GameUI : MonoBehaviour
    {
        ...
        public void SetTimer(bool isActive, float time)
        {
            ...
            if ((!TutorialManager.Instance || !TutorialManager.Instance.enabled) && isActive && time <= 0 &&
                !levelCompleteDisplay.activeSelf)
            {
        ...
        public void Update()
        {
            ...
            redBar.sizeDelta =
                new Vector2(resourceBarWidth * GameController.Instance.bot.GetResourcePercent(ResourceType.Red),
                    barSize.y);
            redResources.text = Mathf.RoundToInt(GameController.Instance.bot.storedRed).ToString();
            redBurnRate.text =
                "-" + Mathf.CeilToInt(GameController.Instance.bot.GetBurnRate(ResourceType.Red)).ToString() + "/s";
            //redResources.text += " (+" + Mathf.RoundToInt(GameController.Instance.bot.hangarRed).ToString() + ")";

            blueBar.sizeDelta =
                new Vector2(resourceBarWidth * GameController.Instance.bot.GetResourcePercent(ResourceType.Blue),
                    barSize.y);
            blueResources.text = Mathf.RoundToInt(GameController.Instance.bot.storedBlue).ToString();
            blueBurnRate.text =
                "-" + Mathf.CeilToInt(GameController.Instance.bot.GetBurnRate(ResourceType.Blue)).ToString() + "/s";
            //blueResources.text += " (+" + Mathf.RoundToInt(GameController.Instance.bot.hangarBlue).ToString() + ")";

            greenBar.sizeDelta =
                new Vector2(resourceBarWidth * GameController.Instance.bot.GetResourcePercent(ResourceType.Green),
                    barSize.y);
            greenResources.text = Mathf.RoundToInt(GameController.Instance.bot.storedGreen).ToString();
            greenBurnRate.text =
                "-" + Mathf.CeilToInt(GameController.Instance.bot.GetBurnRate(ResourceType.Green)).ToString() + "/s";
            //greenResources.text += " (+" + Mathf.RoundToInt(GameController.Instance.bot.hangarGreen).ToString() + ")";

            yellowBar.sizeDelta =
                new Vector2(resourceBarWidth * GameController.Instance.bot.GetResourcePercent(ResourceType.Yellow),
                    barSize.y);
            yellowResources.text = Mathf.RoundToInt(GameController.Instance.bot.storedYellow).ToString();
            yellowBurnRate.text =
                "-" + Mathf.CeilToInt(GameController.Instance.bot.GetBurnRate(ResourceType.Yellow)).ToString() + "/s";
            //yellowResources.text += " (+" + Mathf.RoundToInt(GameController.Instance.bot.hangarYellow).ToString() + ")";

            greyBar.sizeDelta =
                new Vector2(resourceBarWidth * GameController.Instance.bot.GetResourcePercent(ResourceType.Grey),
                    barSize.y);
            greyResources.text = Mathf.RoundToInt(GameController.Instance.bot.storedGrey).ToString();
            greyBurnRate.text =
                "-" + Mathf.CeilToInt(GameController.Instance.bot.GetBurnRate(ResourceType.Grey)).ToString() + "/s";
