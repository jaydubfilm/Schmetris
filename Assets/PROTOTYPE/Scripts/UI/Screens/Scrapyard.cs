...
//Bot buidling/purchasing area
    public class Scrapyard : MonoBehaviour
    {
        ...
        void SnapBricksToBot()
        {
            ...
            for (int i = 0; i <= GameController.Instance.bot.maxBotRadius; i++)
            {
                for (int b = -i; b <= i; b++)
                {
                    ...
                    {
                        new Vector2Int(startPoint.x + b, startPoint.y + i),
                        new Vector2Int(startPoint.x - i, startPoint.y + b),
                        new Vector2Int(startPoint.x - b, startPoint.y - i),
                        new Vector2Int(startPoint.x + i, startPoint.y - b)
                    };
                    foreach (Vector2Int coords in testCoords)
                    {
                        ...
                        {
                            if (botBricks[coords.x + coords.y * botMap.GetLength(1)].GetComponent<Image>().color !=
                                Color.clear)
                            {
        ...
        bool IsCoreConnected(Vector2Int coords, List<Vector2Int> checkedCoords)
        {
            ...
            {
                new Vector2Int(coords.x, coords.y + 1), new Vector2Int(coords.x, coords.y - 1),
                new Vector2Int(coords.x + 1, coords.y), new Vector2Int(coords.x - 1, coords.y)
            };
            ...
            while (startPoint != target)
            {
                ...
                if (testBrick.color == Color.clear)
                {
                    ...
                    if (containerCheck != -1)
                    {
                        ...
                        containers[containerCheck].coords =
                            new Vector2Int(coordCheck - yCol * botMap.GetLength(1), yCol);
                        containerObjects[containerCheck].transform.GetChild(1).localEulerAngles =
                            new Vector3(0, 0, containers[containerCheck].openDirection);
        ...
        List<Vector2Int> GetRouteToCore(Vector2Int coords, List<Vector2Int> routeCoords)
        {
            ...
            for (int i = 0; i < adjacentCoords.Length; i++)
            {
                if (!newRouteCoords.Contains(adjacentCoords[i]) &&
                    Vector2.Distance((Vector2) adjacentCoords[i], (Vector2) GameController.Instance.bot.coreV2) <
                    Vector2.Distance((Vector2) coords, (Vector2) GameController.Instance.bot.coreV2))
                {
                    List<Vector2Int> compareRoute = GetRouteToCore(adjacentCoords[i], newRouteCoords);
                    if (compareRoute.Count != 0 &&
                        compareRoute[compareRoute.Count - 1] == GameController.Instance.bot.coreV2 &&
                        (shortestRoute.Count == 0 || compareRoute.Count < shortestRoute.Count))
                    {
                ...
                for (int x = 0; x < botMap.GetLength(0); x++)
                {
                    ...
                    if (botMap[x, y])
                    {
                        ...
                        foreach (ContainerData container in containers)
                        {
                            ...
                            {
                                isContainer = true;
                                newTile.transform.GetChild(1).localEulerAngles =
                                    new Vector3(0, 0, container.openDirection);
                            }
                        }
        ...
        void UpdateResources()
        {
            ...
            blueBar.sizeDelta = new Vector2(maxBarWidth * (maxCapacity > 0 ? currentBlue / maxCapacity : 0),
                blueBar.sizeDelta.y);
            ...
            yellowBar.sizeDelta = new Vector2(maxBarWidth * (maxCapacity > 0 ? currentYellow / maxCapacity : 0),
                yellowBar.sizeDelta.y);
            ...
            greenBar.sizeDelta = new Vector2(maxBarWidth * (maxCapacity > 0 ? currentGreen / maxCapacity : 0),
                greenBar.sizeDelta.y);
            ...
            greyBar.sizeDelta = new Vector2(maxBarWidth * (maxCapacity > 0 ? currentGrey / maxCapacity : 0),
                greyBar.sizeDelta.y);
            ...
            fuelBar.sizeDelta = new Vector2(maxBarWidth * (maxCapacity > 0 ? currentFuel / maxCapacity : 0),
                fuelBar.sizeDelta.y);
        ...
        private void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0) && canMove)
            {
                ...
                for (int i = 0; i < targets.Count; i++)
                {
                    ...
                    //An existing brick is being edited
                             ...
                             targets[i].gameObject.GetComponent<Image>().color != Color.clear)
                    {
                        isMarketBrick = false;
            ...
            else if (Input.GetMouseButtonUp(0) && canMove)
            {
                ...
                else if (selectedBrick)
                {
                    ...
                    for (int i = 0; i < targets.Count; i++)
                    {
                        if (botBricks.Contains(targets[i].gameObject) &&
                            targets[i].gameObject.GetComponent<Image>().color == Color.clear)
                        {
                            if (isMarketBrick)
                            {
                                if (transactionAmount >= GetBrickCost(selectedBrick.GetComponent<Image>().sprite))
                                {
                                    ...
                                    targets[i].gameObject.GetComponent<Image>().sprite =
                                        selectedBrick.GetComponent<Image>().sprite;
                                    ...
                                    if (IsOpenContainer(targets[i].gameObject))
                                    {
                                        ...
                                        newContainer.coords = new Vector2Int(coordCheck - yCol * botMap.GetLength(1),
                                            yCol);
            ...
            else if (Input.GetMouseButton(0) && canMove)
            {
                ...
                if (holdingScreenTimer >= maxTapTimer)
                {
                    if (botBrick && !selectedBrick)
                    {
                        ...
                        {
                            Vector2 newBotPos = botDisplay.GetComponent<RectTransform>().anchoredPosition +
                                                new Vector2(Input.mousePosition.x - prevMousePos.x,
        ...
        {
            canMove = false;
            tempMoneyAmount = uncommittedBricks.Contains(sellBrick)
                ? GetBrickCost(sellBrick.GetComponent<Image>().sprite)
                : GetBrickSell(sellBrick.GetComponent<Image>().sprite);
            ...
            foreach (GameObject Resource in convertableParts)
            {
                ...
                for (int i = 0; i < resourceBrick.spriteArr.Length; i++)
                {
                    if (targetPart == resourceBrick.spriteArr[i])
                    {
                        ...
                        tempYellowAmount = Resource.GetComponent<Yellectrons>()
                            ? Resource.GetComponent<Yellectrons>().maxResource[i]
                            : 0;
                        tempGreyAmount = Resource.GetComponent<Greyscale>()
                            ? Resource.GetComponent<Greyscale>().maxResource[i]
                            : 0;
                        tempGreenAmount =
                            (Resource.GetComponent<Repair>() && Resource.GetComponent<Repair>().maxResource.Length > 0)
                                ? Resource.GetComponent<Repair>().maxResource[i]
                                : 0;
                        tempBlueAmount = Resource.GetComponent<BlueSaltGun>()
                            ? Resource.GetComponent<BlueSaltGun>().maxResource[i]
                            : 0;
                        break;
                    }
                }
        ...
        public void BrickOptions()
        {
            ...
            foreach (CraftedPart TempPart in tempUpgrades)
            {
                ...
                upgradeButton.GetComponent<Text>().text =
                    "UPGRADE: " + targetUpgrade + " (" + GetUpgradeString(targetUpgrade) + ")";
            ...
            if (coreBrick != selectedPart)
            {
                ...
                foreach (GameObject Resource in convertableParts)
                {
                    ...
                    for (int i = 0; i < resourceBrick.spriteArr.Length; i++)
                    {
                        if (targetPart == resourceBrick.spriteArr[i])
                        {
                            return Resource.GetComponent<Fuel>() || Resource.GetComponent<Yellectrons>() ||
                                   Resource.GetComponent<Greyscale>() ||
                                   (Resource.GetComponent<Repair>() &&
                                    Resource.GetComponent<Repair>().maxResource.Length > 0) ||
                                   Resource.GetComponent<BlueSaltGun>();
                        }
                    }
                }
            }
        ...
        bool CanUpgrade(Sprite selectedPart, CraftedPart selectedUpgrade)
        {
            for (int i = 0; i < selectedUpgrade.basePartToCraft.Length; i++)
            {
                ...
                {
                    return (currentBlue + excessBlue) >= selectedUpgrade.blueToCraft[i] &&
                           (currentFuel + excessRed) >= selectedUpgrade.redToCraft[i] &&
                           (currentGreen + excessGreen) >= selectedUpgrade.greenToCraft[i] &&
                           (currentYellow + excessYellow) >= selectedUpgrade.yellowToCraft[i] &&
                           (currentGrey + excessGrey) >= selectedUpgrade.greyToCraft[i] &&
                           GameController.Instance.money >= selectedUpgrade.moneyToCraft[i];
                }
            }
        ...
        public void ConfirmSellAll()
        {
            ...
            for (int i = 0; i < botBricks.Count; i++)
            {
                ...
                if (botImage.color != Color.clear && botBricks[i] != coreBrick &&
                    brickType == GetBrickName(botImage.sprite, false))
                {
                    ...
                    brickType == GetBrickName(botImage.sprite, false))
                {
                    ...
                    foreach (GameObject Resource in convertableParts)
                    {
                        ...
                        for (int r = 0; r < resourceBrick.spriteArr.Length; r++)
                        {
                            ...
                            {
                                currentFuel += Resource.GetComponent<Fuel>()
                                    ? Resource.GetComponent<Fuel>().maxFuelArr[r]
                                    : 0;
                                currentBlue += Resource.GetComponent<BlueSaltGun>()
                                    ? Resource.GetComponent<BlueSaltGun>().maxResource[r]
                                    : 0;
                                currentYellow += Resource.GetComponent<Yellectrons>()
                                    ? Resource.GetComponent<Yellectrons>().maxResource[r]
                                    : 0;
                                currentGreen +=
                                    (Resource.GetComponent<Repair>() &&
                                     Resource.GetComponent<Repair>().maxResource.Length > 0)
                                        ? Resource.GetComponent<Repair>().maxResource[r]
                                        : 0;
                                currentGrey += Resource.GetComponent<Greyscale>()
                                    ? Resource.GetComponent<Greyscale>().maxResource[r]
                                    : 0;
                                break;
                            }
                        }
        ...
        int GetBrickCost(Sprite targetBrick)
        {
            foreach (MarketData marketCheck in marketData)
            {
                ...
                for (int i = 0; i < marketCheck.brickLevels.Length; i++)
                {
                    ...
                    {
                        return brickCheck.GetComponent<CraftedPart>()
                            ? Mathf.RoundToInt(GameController.Instance.costMultiplier *
                                               marketCheck.brickLevels[i].buyPrice)
                            : marketCheck.brickLevels[i].buyPrice;
                    }
                }
            }
            ...
            foreach (MarketData marketCheck in marketData)
            {
                ...
                for (int i = 0; i < marketCheck.brickLevels.Length; i++)
                {
                    if (brickCheck.spriteArr[i] == targetBrick)
                    {
                        return brickCheck.GetComponent<CraftedPart>()
                            ? Mathf.RoundToInt(GameController.Instance.costMultiplier *
                                               marketCheck.brickLevels[i].sellPrice)
                            : marketCheck.brickLevels[i].sellPrice;
                    }
                }
            }
