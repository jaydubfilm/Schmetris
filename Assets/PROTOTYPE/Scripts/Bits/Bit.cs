...
namespace StarSalvager.Prototype
{
    ...
    public class Bit : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            screenOffset =
                new Vector2Int(Mathf.RoundToInt((transform.position.x - parentOffsetV3.x) / ScreenStuff.colSize),
                    Mathf.RoundToInt((transform.position.y - parentOffsetV3.y) / ScreenStuff.rowSize));
        ...
        void FixedUpdate()
        {
            if (!hasBounced && CanCollideFlag && !GetComponent<ParasiteBit>())
            {
                ...
                RaycastHit2D rH = Physics2D.BoxCast(transform.position, Vector2.one * ScreenStuff.colSize, 0,
                    Vector2.down,
                    Mathf.Max(ScreenStuff.colSize / 2.0f,
                        parentBlock
                            ? parentBlock.rb.velocity.magnitude * Time.fixedDeltaTime
                            : ScreenStuff.colSize / 2.0f), brickMask);
                if (rH.collider != null)
                {
