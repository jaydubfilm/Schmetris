...
namespace StarSalvager.UI.Scrapyard
{
    public class DroneDesignUI : MonoBehaviour
    {
        ...
        public void ShowPartDetails(bool show, in ScrapyardPart scrapyardPart)
        {
            ...
            var screenPoint = CameraController.Camera.WorldToScreenPoint(scrapyardPart.transform.position + Vector3.right);
        ...
        private string GetAltDetails(in PartData partData, in PartRemoteData partRemoteData)
        {
            float GetPatchMultiplier(in PatchData[] patches, PATCH_TYPE patchType)
            {
                ...
                {
                    if(!patchRemoteData.GetRemoteData(patchData.Type).TryGetValue(patchData.Level, PartProperties.KEYS.Multiplier, out float multiplier))
                        continue;
            ...
            switch (partRemoteData.partType)
            {
                ...
                case PART_TYPE.GUN:
                {
                    ...
                    var dps = Mathf.RoundToInt(damage * GetPatchMultiplier(partData.Patches, PATCH_TYPE.DAMAGE) / (cooldown * GetPatchMultiplier(partData.Patches, PATCH_TYPE.FIRE_RATE)));
